...
describe('测试', function() {
      ...
      describe('卸载云硬盘', function() {
        ...
        beforeEach(function() {
		  ...
		  moreAction.click();
		  let loadVolumes  = element(by.buttonText('卸载云硬盘'));
		  loadVolumes.click();
		  let choseClick=element(by.model('disk.volumeId')); 
	 	  choseClick.click()
	 	  let choseFirst=element.all(by.css('.ui-select-bootstrap .ui-select-choices-row>a')).get(0); 
	 	  choseFirst.click();
	 	  let sureClick=element(by.css('[ng-click="confirm(disk.volumeName,detachdiskForm)"]')); 
	 	  sureClick.click()
