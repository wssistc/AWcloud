cardCtrl.$inject=["$scope", "$http","$timeout", "$location", "$routeParams", "$interval","$translate"];
export function cardCtrl($scope, $http,$timeout, $location, $routeParams, $interval,$translate){
    var self = $scope;
    self.bondsType = [
        {
            name:"主备",
    ...
    if(localStorage.cardCheckedItems){
        cardCheckedItems = JSON.parse(localStorage.cardCheckedItems)[0];
    ...
    $(".new-bond")[0].ondrop = function(event){
        ...
        function addBonds(event){
            ...
            self.$apply()
            /*给予drop监听*/
            // $(".bond-desc")[$(".bond-desc").length-1].addEventListener('drop',function(event){
            //     event.preventDefault();
            ...
            /*开始拖动事件*/
            // $(".bonddrag")[$(".bonddrag").length-1].addEventListener('dragstart',function(event){
            //     dragName = 'bonds';
    ...
    function addBondsListen(){
        ...
        for(var i=0;i<allBonds.length;i++){
            // allBonds[i].addEventListener('drop',function(event){
            //     event.preventDefault();
            //     bondsIndex= $(this).attr('indexNum');
            //     cardSuperposition(bondsIndex)
            // })
        ...
        for(var i=0;i<allBondsLi.length;i++){
            // allBondsLi[i].addEventListener('dragstart',function(event){
            //     dragName = 'bonds';
            //     bondsIndex= $(this).find(".bond-desc").attr('indexNum');
            //     event.dataTransfer.setData("text/html",$(this).prop("outerHTML"));
            // },false)
    ...
    self.saveCards = function(){
        ...
        var cardCheckedItems = JSON.parse(localStorage.cardCheckedItems)
        ...
        // self.toolSuccess = true;
        // $timeout(function(){
        //      self.toolSuccess = false;   
        // },3000)
    }
}