...
angular.module("app", ["ngRoute", "pascalprecht.translate","ngMessages", "ui.bootstrap","ui.bootstrap.tpls", "ngSanitize","ui.select","ngTable"])
    ...
    .config(["$routeProvider", "$locationProvider", "$httpProvider", "$translateProvider",
        function($routeProvider, $locationProvider, $httpProvider, $translateProvider) {
                    ...
                    return {
                        request: function(config) {
                            if (config.url.indexOf("awstack-user") > -1) {
                                ...
                                //config.url = "http://192.168.137.77:8080/awstack-user"+user_url[1];
                            }
                        ...
                        response: function(res) {
                            if (res.data.code === "00010101" || res.data.code === "00010105") {
                                var regionKeyCode = $location.path().replace("/","")
                                location.replace("#/"+regionKeyCode);
                            }
                            return res;
                        },
            ...
            $routeProvider
                ...
                .when("/info/cards/:regionKey", {
                    templateUrl: "/js/datacenter/tmpl/cardconfigure.html",
                    controller: "cardCtrl",
                    reloadOnSearch: false
                })
                .when("/info/disks/:regionKey", {
                    templateUrl: "/js/datacenter/tmpl/disksconfigure.html",
                    controller: "disksCtrl",
                    reloadOnSearch: false
                })
                .when("/info/stepone/:regionKey", {
                    templateUrl: "/js/datacenter/tmpl/stepone.html",
                    controller: "steponeCtrl",
                    reloadOnSearch: false
                })
                .when("/info/steptwo/:regionKey", {
                    templateUrl: "/js/datacenter/tmpl/steptwo.html",
                    controller: "steptwoCtrl",
                    reloadOnSearch: false
                })
                .when("/info/stepthree/:regionKey", {
                    templateUrl: "/js/datacenter/tmpl/stepthree.html",
                    controller: "stepthreeCtrl",
                    reloadOnSearch: false
                })
                .when("/info/stepfour/:regionKey", {
                    templateUrl: "/js/datacenter/tmpl/stepfour.html",
                    controller: "stepfourCtrl",
                    reloadOnSearch: false
                })
                .when("/:regionKey", {
                    templateUrl: "/js/datacenter/tmpl/loading.html",
                ...
                .when("/complete/:regionKey", {
                    templateUrl: "/js/datacenter/tmpl/complete.html",
    ...
    .controller("loginCtrl", ["$scope","$location", "$http", function($scope,$location, $http) {
        ...
        self.getRegionFun = function(regionKeyCode) {
            $http({
                method: "POST",
                url: "awstack-user/v1/back/enterprises/signinByRegion",
                headers: {'X-Register-Code':regionKeyCode}
                ...
                if (res.code === "0") {
                    ...
                    if (res.data.data.status==1 || res.data.data.status==2 || res.data.data.status==3) {
                        $location.path("/complete/"+regionKeyCode).replace();
                    } else{
                        $location.path("/info/stepone/"+regionKeyCode).replace();
                    }
                } else {
                    self.lgError = true;
                }
        ...
        self.getRegionFun(regionKeyCode)
    }])
    ...
    .controller("completeCtrl", ["$scope","$http","$location","$translate","$timeout", function(scope,$http,$location,translate,$timeout) {
        ...
        var regionKey = localStorage.LOGINS?JSON.parse(localStorage.LOGINS).regionKey:regionkeyCode;
    ...
    .controller("mainCtrl", ["$scope", "$rootScope", "$routeParams","$location","$http","$translate", function($scope, $rootScope, $routeParams,$location,$http,translate) {
        ...
        $rootScope.$on("$routeChangeStart",function(e,next){
            ...
            var pathData = $location.path().split("/");
            var regionkeyCode = pathData[pathData.length-1];
            if(!localStorage.$AWCONFLOGINED){
                $location.path(regionkeyCode).replace();
            }
            if(localStorage.LOGINS){
                var regionKey = JSON.parse(localStorage.LOGINS).regionKey;
                /*当切换数据中心进来时进行重新登录操作*/
                if(regionkeyCode!=regionKey){
                   $location.path(regionkeyCode).replace();
                }
                var data={
                    "regionKey":regionKey
                ...
                $http({
                    ...
                    if (result) {
                        ...
                        if(registerStatus==1||registerStatus==2){
                            $location.path("/complete/"+regionKey).replace();
                        }
                    }
                })
            }
        ...
        function changeSiteTitle(path) {
            ...
            var conTitle = titles.length == 2?titles[0]:"";
            console.log(conTitle)
            if(title==':regionKey'&&conTitle!='complete'){
                var trans_title = translate.instant("aws.siteTitle.regionslogion");
            }else{
                if(conTitle=='complete'){
                    title = 'complete';
                }
        ...
        $rootScope.$on("$routeChangeSuccess", function(a, cur) {
            ...
            var regionKeyCode = localStorage.LOGINS?JSON.parse(localStorage.LOGINS).regionKey:"";
            if(cur.originalPath){
                self.isLogin = cur.originalPath == "/:regionKey" ? true : false;
                self.isInfo = cur.originalPath.indexOf("/info") > -1 ? true : false;
                self.isStepfour = cur.originalPath == "/info/stepfour/:regionKey" ? true : false;
                changeSiteTitle(cur.originalPath);
            }
            if (!cur.originalPath || cur.originalPath == ("/:regionKey")) {
                localStorage.removeItem("$AWCONFLOGINED");
            }
            if(cur.originalPath!="/info/stepone/:regionKey"){
                self.isSetpOne = true;
            }
            switch (cur.originalPath) {
            case "/info/stepthree/:regionKey":
                self.stepOne = false;
                self.stepTwo = true;
            ...
            case "/info/stepfour/:regionKey":
                ...
                break;
            case "/info/stepone/:regionKey":
            case "/info/cards/:regionKey":
            case "/info/disks/:regionKey":
                self.stepOne = true;
                self.stepTwo = false;
                self.stepThree = false;
                self.stepFour = true;
