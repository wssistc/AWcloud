import "ip";
import { zh_CN } from "../register/i18n/zh_CN";
import { en_US } from "../register/i18n/en_US";
import * as steponeCtrl from "../register/controllers/steponeCtrl";
import * as steptwoCtrl from "../register/controllers/steptwoCtrl";
import * as stepthreeCtrl from "../register/controllers/stepthreeCtrl";
import * as stepfourCtrl from "../register/controllers/stepfourCtrl";
import * as disksCtrl from "../register/controllers/disksCtrl";
import * as cardCtrl from "../register/controllers/cardCtrl";
angular.module("app", ["ngRoute", "pascalprecht.translate","ngMessages", "ui.bootstrap","ui.bootstrap.tpls", "ngSanitize","ui.select","ngTable"])
    ...
    .config(["$routeProvider", "$locationProvider", "$httpProvider", "$translateProvider",
        function($routeProvider, $locationProvider, $httpProvider, $translateProvider) {
            ...
            $routeProvider
                ...
                .when("/", {
                    templateUrl: "/js/register/tmpl/loading.html",
    ...
    .controller("loginCtrl", ["$scope", "$http","$location","$interval","$window", function($scope, $http,$location,$interval,$window) {
        var self = $scope;
        self.lgError = false;
        localStorage.clear();  
        ...
        getLogin();
        var getInstallStatus = $interval(function() {
            if (!$window.isInstall) {
                getLogin()
            } else {
                $interval.cancel(getInstallStatus);
                getInstallStatus = undefined; 
            }
        }, 10000);
    ...
    .controller("completeCtrl", ["$scope","$http","$location","$translate","$interval","$window","$route","$timeout", function(scope,$http,$location,translate,$interval,$window,$route,$timeout) {
        ...
        if (localStorage.LOGINS && localStorage.INSTALLSTATUS!="3") {
            if ("WebSocket" in window) {
                ...
                //let url = "ws://192.168.140.126:8001";
                ...
                websocket.onmessage = function(event) {
                    if(scope.logOut){
                        self.logMessages.unshift(event.data);
                    }
                    if(localStorage.INSTALLSTATUS=="3"){
                        self.passed = true;
                        self.startUp = false;
                            ...
                            if(!$window.vmStatus){
                                $http({
                                    ...
                                    if(res&&res.data.data.data==3){
                                        ...
                                        self.components_ceph = true;
                                        self.complete.status = "color-9";
                                        $window.vmStatus = true;
                                        localStorage.INSTALLSTATUS = 3;
                                        $timeout(function(){
                                            self.vmShutDown = false;
                                            window.location.reload()
                                        },1500)
                                    }else if(res&&res.data.data.data==1){
                                        $window.vmStatus = true;
                                        $timeout(function(){
                                            self.vmShutDown = false;
                                            window.location.reload()
        ...
        self.reseConfirm = function(){
            ...
            if (localStorage.LOGINS) {
                ...
                data.push({"regionKey":localStorage.LOGINS?JSON.parse(localStorage.LOGINS).regionKey:'FFFFF'});
                $http({
                    method:"DELETE",
                    headers:{
                ...
                ele.on("keyup",function(){
                    if(model.$viewValue){
                        $window.username = $timeout(function(){
                            regsrv.checkUserName({userName:model.$viewValue,regionKey:localStorage.LOGINS?JSON.parse(localStorage.LOGINS).regionKey:'FFFFF'}).then(function(result){
                                if(result.data.code=="0"){
