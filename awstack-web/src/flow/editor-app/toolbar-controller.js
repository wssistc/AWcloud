...
angular.module('activitiModeler')
    .controller('ToolbarController', ['$scope', '$http', '$modal', '$q', '$rootScope', '$translate', '$location', function ($scope, $http, $modal, $q, $rootScope, $translate, $location) {
        ...
        $scope.toolbarButtonClicked = function(buttonIndex) {
            /*当流程名称，名称，描述没有填写的情况不保存*/
            $scope.edit0 = true;
            $scope.edit1 = true;
            var flowProcess=$scope.selectedItem.properties;
           
            var pattern = new RegExp("^([A-Za-z]|[\u4E00-\u9FA5])+$") ;
            /*定位保存按钮*/
            if(buttonIndex==0){
                for(var i=0;i<2;i++){
                    if($scope.selectedItem.properties[i].value==""){
                        ...
                        if(i==0){
                           console.log(pattern.test($scope.selectedItem.properties[i].value))
                            if(!pattern.test($scope.selectedItem.properties[i].value) && $scope.selectedItem.properties[i].title =='Id' ) {
                ...
                if(!$scope.edit0||!$scope.edit1){
                    $scope.$emit('tostencil', true);
                    return
                }else{
                    $scope.$emit('tostencil', false);
                }
            }
