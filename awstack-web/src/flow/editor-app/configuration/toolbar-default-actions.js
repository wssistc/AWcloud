...
var SaveModelCtrl = [ '$rootScope', '$scope', '$http', '$route', '$location',"$translate",
    function ($rootScope, $scope, $http, $route, $location,$translate) {
    var pattern = new RegExp("^([a-zA-Z0-9]|[\u4E00-\u9FA5]|\-|\_)*$") ;
    ...
    $scope.$watch(function(){
        return $scope.saveDialog.name
    },function(value){
        $scope.error= false;
       if(value == ""){
        $scope.nameError = "必须填"
        $scope.nameErrored = true;
       }
       else if(value.length>20){
        $scope.nameError = "名称最多为20位"
        $scope.nameErrored = true;
       }
       else if(!pattern.test(value)){
        $scope.nameError = "只能输入字母、数字、汉字、横线以及下划线"
        $scope.nameErrored = true;
       }
       else{
        $scope.nameError = ""
        $scope.nameErrored = false;
       }
    })
    $scope.$watch(function(){
        return $scope.saveDialog.description
    },function(value){
       
      
      if(value.length>70){
        $scope.descError = "描述最多为70位"
        $scope.descErrored = true;
       }else{
        $scope.descError = ""
        $scope.descErrored = false;
       }
      
    })
    $scope.save = function (successCallback) {

        if (!$scope.saveDialog.name || $scope.saveDialog.name.length == 0 ||  $scope.nameErrored ||$scope.descErrored) {
            return;
        }
        ...
        $http({    method: 'PUT',
            ...
            .error(function (data, status, headers, config) {
                if(status == 500){
                    $scope.errorInfo = "该名称已被使用"
                }else{
                    $scope.errorInfo =  $translate.instant("MODEL.SAVE.ERROR")
                }
                $scope.error = {};
