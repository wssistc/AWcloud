(function (context) {
  ...
  TheGraph.vm_status=function(){
    console.log("123")
   // console.log(this.props.node.metadata)
    return "node-rect drag"
  }
  ...
  TheGraph.Node = React.createFactory( React.createClass({
    ...
    render: function() {
      ...
      //当虚拟机关机或异常时改变node-icon的颜色
      if(this.props.node.metadata.vm_status&&(this.props.node.metadata.vm_status=="stopped"||this.props.node.metadata.vm_status=="error")){
        var nodeClassName;
        if(this.props.node.metadata.vm_status=="stopped"){
          var nodeClassName="icon shut-down-node-icon drag";
        }else if(this.props.node.metadata.vm_status=="error"){
          var nodeClassName="icon error-node-icon drag";
        }
        var node_icon= {
          ref: "icon",
        ...
        if (this.props.iconsvg && this.props.iconsvg !== "") {
          var iconSVGOptions = TheGraph.merge(node_iconsvg, {
              src: this.props.iconsvg,
              x: TheGraph.config.nodeRadius - 4,
              y: TheGraph.config.nodeRadius - 4,
              width: this.props.width - 10,
          ...
          var iconOptions = TheGraph.merge(node_icon, {
              x: this.props.width / 2,
              y: this.props.height / 2,
              children: icon });
          iconContent = TheGraph.factories.node.createNodeIconText.call(this, iconOptions);
      ...
      //修改vm状态为关机和异常时node的border
      if(this.props.node.metadata.vm_status&&(this.props.node.metadata.vm_status=="stopped"||this.props.node.metadata.vm_status=="error")){
        var nodeClassName;
        if(this.props.node.metadata.vm_status=="stopped"){
          var nodeClassName="shut-down-node-border drag";
        }else if(this.props.node.metadata.vm_status=="error"){
          var nodeClassName="error-node-border drag";
        }
        var node_border={
          className: nodeClassName,
          rx: TheGraph.config.nodeRadius,
        ...
        var borderRectOptions = TheGraph.merge(node_border, { width: this.props.width, height: this.props.height });
      }
      ...
      if(this.props.node.metadata.vm_status&&(this.props.node.metadata.vm_status=="stopped"||this.props.node.metadata.vm_status=="error")){
        ...
        var nodeClassName;
        if(this.props.node.metadata.vm_status=="stopped"){
          var nodeClassName="node-rect-shutdown drag";
        }else if(this.props.node.metadata.vm_status=="error"){
          var nodeClassName="node-rect-error drag";
        }
        var stoppedVm={
          className: nodeClassName,
          x: 3,
        ...
        var innerRectOptions = TheGraph.merge(stoppedVm, { width: this.props.width - 6, height: this.props.height - 6 });
      }else{
