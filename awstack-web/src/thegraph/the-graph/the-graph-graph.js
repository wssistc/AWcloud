(function (context) {
  ...
  TheGraph.Graph = React.createFactory( React.createClass({
        ...
        if (componentName && this.props.library) {
          ...
          for (i=0, len=component.outports.length; i<len; i++) {
            ...
            outports[port.name] = {
              ...
              x: node.metadata.width / (len+1) * (i+1),
              y: node.metadata.height
            };
          }
    ...
    getNodeOutport: function (graph, processName, portName, route, componentName) {
      ...
      if ( !ports.outports[portName] ) {
        ports.outports[portName] = {
          ...
          x: TheGraph.config.nodeWidth / 2,
          y: TheGraph.config.nodeHeight
        };
        this.dirty = true;
      }
    ...
    render: function() {
      //console.log('render', this.state.edgePreview)
      this.dirty = false;
      ...
      var edges = graph.edges.map(function (edge) {
        ...
        var edgeOptions = {
          ...
          sX: source.metadata.x + sourcePort.x,
          sY: source.metadata.y + source.metadata.height,
          tX: target.metadata.x + targetPort.x,
          tY: target.metadata.y,
          label: label,
          route: route,
      ...
      var iips = graph.initializers.map(function (iip) {
        ...
        var tX = target.metadata.x + targetPort.x;
        var tY = target.metadata.y;
      ...
      var inports = Object.keys(graph.inports).map(function (key) {
        ...
        var expEdge = {
          ...
          sX: expNode.x + TheGraph.config.nodeWidth / 2,
          sY: expNode.y + TheGraph.config.nodeHeight,
          tX: privateNode.metadata.x + privatePort.x,
          tY: privateNode.metadata.y + privatePort.y,
          showContext: self.props.showContext
        };
      ...
      var outports = Object.keys(graph.outports).map(function (key) {
        ...
        var expEdge = {
          ...
          sX: privateNode.metadata.x + privatePort.x,
          sY: privateNode.metadata.y + privatePort.y,
          tX: expNode.x + TheGraph.config.nodeWidth / 2,
          tY: expNode.y,
          showContext: self.props.showContext
        };
      ...
      if (edgePreview) {
        ...
        if (edgePreview.from) {
          ...
          edgePreviewOptions = {
            //sX: source.metadata.x + source.metadata.width,
            sX: source.metadata.x + source.metadata.width / 2,
            sY: source.metadata.y + sourcePort.y,
          ...
          edgePreviewOptions = {
            ...
            //tX: target.metadata.x,
            tX: target.metadata.x + target.metadata.width / 2,
            tY: target.metadata.y + targetPort.y,
            route: edgePreview.metadata.route
          };
        }
