var alertmodule = angular.module("alertsrv", ["ngAnimate", "ui.bootstrap"]);
...
alertmodule.service("alertSrv", function($timeout, $sce, $rootScope, $uibModal, $q,$translate) {
    ...
    $rootScope.$on("alert-error", function(e, alert,requestId) {
        self.set(requestId, alert, "error",10000);
    }, $rootScope);
    ...
    $rootScope.$on("alert-success", function(e, alert,requestId) {
        self.set(requestId, isNaN(Number(alert))?alert:$translate.instant("aws.statusCode."+ alert), "success", 3000);
    }, $rootScope);
